...
local function PabShapeActive(shape)
    return shape ~= nil and shape ~= "none" and shape ~= "cropped"
end
...
local function BuildStyle(isBuff, cfg)
    ...
    local iconSize = PabSnap(PabShapedSize(rawIconSize, cfg.iconShape))
...
local function BuildGroupLayout(cfg, gap, rowGap, size)
    ...
    size = PabSnap(PabShapedSize(rawSize, cfg.iconShape))
    gap = PabSnap(gap)
...
local function RenderPreviewIcons(box, icons, isBuff, cfg, fontPath, pool)
    ...
    do
        ...
        -- line and later rows wrap back to the bar's own edge. Cropped icons
        -- retain their shorter height on the relevant layout axis.
        local runningX, runningY = {}, 0
        for r = 0, rows - 1 do runningX[r] = 0 end
        for i = 1, total do
            local r = math.floor((i - 1) / cols)
            colOffset[i] = runningX[r] or 0
            runningX[r] = (runningX[r] or 0) + (vertical and slotHeight[i] or slotSize[i]) + pad
